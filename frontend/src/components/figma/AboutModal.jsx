...
export default function AboutModal({ isOpen, onClose, t }) {
  ...
  return (
    <>
      ...
      <div className="modal-overlay-wb" role="dialog" aria-modal="true" aria-label={t.aboutTitle}>
        <div className="modal-card" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body-wb" style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            <div className="about-mascot"><img src={blueCharacter} alt="Blue mascot" /></div>
            <p className="about-text">{t.aboutText}</p>
          </div>
          <div className="modal-footer-wb">
            <button className="btn-primary-wb" style={{ width: '100%' }} onClick={onClose}>{t.close}</button>
          </div>
        </div>
      </div>
    </>
